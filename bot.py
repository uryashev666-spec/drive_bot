...
if sys.platform == "win32":
    ...
    # Настройка stdout/stderr для корректного отображения русских символов
...
async def message_handler(message: types.Message):
    ...
    if match_btn(text, "Записаться на занятие"):
        ...
        await message.answer(
            "📅 Шаг 1: Выберите день для занятия. Слоты с ❌ или 🚫 недоступны для записи.",
            reply_markup=markup
        )
        return
...
async def auto_update_code():
    """
    Автоматическая проверка обновлений кода с GitHub каждые 5 минут.
    При обнаружении изменений - мягкий перезапуск бота.
    """
    while True:
        try:
            await asyncio.sleep(300)  # Проверка каждые 5 минут (300 секунд)
            
            logging.info("🔍 Проверка обновлений с GitHub...")
            
            async with aiohttp.ClientSession() as session:
                async with session.get(GITHUB_RAW_URL) as response:
                    if response.status == 200:
                        new_code = await response.text(encoding='utf-8')
                        ...
                        if new_code != current_code:
                            logging.info("✅ Найдено обновление! Применяю...")
                            
                            # Создаём backup
                            try:
                                with open(f"{__file__}.backup", "w", encoding="utf-8") as f:
                            ...
                            try:
                                await bot.send_message(
                                    YOUR_TELEGRAM_ID,
                                    "🔄 Обнаружено обновление кода!\nБот перезапускается через 3 секунды..."
                                )
                            except Exception:
                                pass
                            ...
                            # Мягкий перезапуск через завершение программы
                            ...
                            # Завершаем программу, bat-файл перезапустит
                            sys.exit(1)
                        else:
                            logging.info("✔️ Код актуален, обновлений нет.")
                    else:
                        logging.warning(f"⚠️ Ошибка загрузки с GitHub: {response.status}")
                        
        except Exception as e:
            logging.error(f"❌ Ошибка автообновления: {e}")
            await asyncio.sleep(300)  # Повтор через 5 минут даже при ошибке
